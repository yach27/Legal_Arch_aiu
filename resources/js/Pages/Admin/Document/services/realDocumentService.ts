...
class RealDocumentService {
  ...
  async getAllDocuments(folderId?: number, filters?: DocumentFilters, searchTerm?: string): Promise<Document[]> {
    ...
    console.log(`API call: GET ${this.apiUrl}${endpoint}`);
    const result = await this.apiCall<Document[]>(endpoint);
  ...
  async getDocumentsByFolder(folderId: number): Promise<Document[]> {
    ...
    const result = await this.getAllDocuments(folderId);
    console.log(`getDocumentsByFolder result for folder ${folderId}:`, result);
    return result;
  }
